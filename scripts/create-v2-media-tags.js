 ...
 * 
 * Uruchom: node scripts/create-v2-media-tags.js
 * 
...
async function createMediaAssets() {
  ...
  for (const template of mediaAssetTemplates) {
    try {
      ...
      console.error(`❌ Błąd przy tworzeniu "${template.title}":`, error.message)
    }
  }
...
createMediaAssets().catch(console.error)
